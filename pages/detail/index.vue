<template>
	<view class="index-content" style="background: white;">
		<view class="top-bckground">
			...
			<!-- #ifdef APP-PLUS -->
			<view class="index-coupon has-pd-10" style="margin:200upx 8upx 180upx 8upx ;background: #FFFFFF">

				<!-- #endif -->
				<!-- #ifndef APP-PLUS -->
				<view class="index-coupon has-pd-10" style="margin:180upx 8upx 180upx 8upx ;background: #FFFFFF">

					<!-- #endif -->
					<view class="category" v-if="category[TabCur].banner.length > 0">
						<view class="list" style="background: #FFFFFF;">
					...
					<view style="background: #F8F8F8;" class="goods-list" v-if="category[TabCur].orderList.length > 0">
						<orange-goods-list v-for="(item, index18) in category[TabCur].orderList" :tkmoney="item.tkmoney" :tkmoneys="item.tkmoneys"
...
<script>
	...
	export default {
		...
		methods: {
			...
			toCategory(son_name) {
				uni.navigateTo({
					url: '/pages/search/search?keywords=' + son_name
				});
			},
