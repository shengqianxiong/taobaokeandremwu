	...
	<view class="register">
		<view class="content">
			<view class="main" style="margin-top: 60upx;">
				...
				<wInput v-show='show1' v-model="invitation" type="number" isShowGet ref="setNumberCode" @setNumberCode="isShowGet()"
		...
		methods: {
			...
			sendMsg() {
				...
				if (!phoneData) {
					...
					this.$Request.getT("/appLogin/sendMsg/" + phoneData + "/login").then(res => {
						if (res.code === 0) {
			...
			startReg() {
				...
				this.$Request.postJson("/appLogin/loginWeiXinApp", {
					...
					invitationCode: _this.invitation,
					...
					if (res.code === 0) {
						...
						this.$Request.postT('/appLogin/updateClientId?clientId=' + clientid +'&userId=' + res.data.userId).then(res => {
						...
						this.getUserInfo(res.data.userId, res.data.token);
					} else {
