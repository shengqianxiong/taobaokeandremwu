	...
	<view class="register">
		<view class="content">
			<view class="main" style="margin-top: 60upx;">
				...
				<wInput v-show='show' v-model="invitation" type="number" isShowGet ref="setNumberCode" @setNumberCode="isShowGet()"
	...
	export default {
		...
		methods: {
			...
			startReg() {
				...
				_this.isRotate = true;
				this.$Request.postJson("/user/loginTaoBao", {
					pwd: _this.passData,
					phone: _this.phoneData,
					msg: _this.verCode,
					platform: _this.platform,
					invitation: _this.invitation,
					imageUrl: _this.$queue.getData('tbImg'),
					nickName:_this.$queue.getData('tbNick'),
					openid: _this.$queue.getData("tbopenid")
				}).then(res => {
					if (res.status === 0) {
