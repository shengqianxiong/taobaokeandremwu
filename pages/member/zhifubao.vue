<template>
	<view class="container">
		...
		<button class="confirm-btn" @click="toLogin" :disabled="logining">绑定账户</button>
		...
		methods: {
			...
			toLogin() {
				...
				if (!mobile) {
					...
					this.$Request.postT("/app/insertZhiFuBao?userId=" + userId + "&zhifubao=" + mobile + "&zhifubaoName=" + userName).then(
						res => {
							if (res.code === 0) {
								this.navBack();
							} else {
